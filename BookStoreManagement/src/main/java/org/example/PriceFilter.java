...
public class PriceFilter implements FilterStrategy {
    // The class should sort the products by Price and return it
    // This implementation uses quicksort as algorithm to sort the arraylist.
    ...
    private void quicksort(ArrayList<Product> products, int lowIndex, int highIndex) {
        ...
        double pivot = products.get(pivotIndex).getSellPrice();
    ...
    private int partition(ArrayList<Product> products, int lowIndex, int highIndex, double pivot) {
        ...
        while(leftPointer < rightPointer) {
            ...
            while (products.get(leftPointer).getSellPrice() <= pivot && leftPointer < rightPointer) {
                leftPointer++;
            }
            while (products.get(rightPointer).getSellPrice() >= pivot && leftPointer < rightPointer) {
                rightPointer--;
            }
